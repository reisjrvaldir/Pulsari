        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '4rem', marginBottom: '3rem' }} className="footer-grid">
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '1.2rem' }}>
              {socials.map(s => (
                s.href
                  ? <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="btn" style={{ padding: '0.5rem 0.75rem', display: 'inline-flex', alignItems: 'center' }} title={s.label}>{s.icon}</a>
